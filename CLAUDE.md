...
- **`typecheck` gates PRs; the full test suite doesn't (yet).** `.github/workflows/ci.yml` runs both jobs on every PR — `typecheck` is blocking, `test` has `continue-on-error: true` until a DO test deadlock is fixed. Run `pnpm typecheck && pnpm test` locally before every push (or use `/preflight`) to catch what CI won't yet gate on.
