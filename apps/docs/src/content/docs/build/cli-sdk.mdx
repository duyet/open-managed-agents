...
- **`@getoma/cli`** — a Node CLI for scripting against the API.
...
<Tabs syncKey="pkg-mgr">
  ...
  <TabItem label="pnpm">
    ```bash
    pnpm add -g @getoma/cli
    ```
  </TabItem>
  <TabItem label="bun">
    ```bash
    bun add -g @getoma/cli
    ```
  </TabItem>
</Tabs>
...
<Aside type="caution" title="@getoma/sdk is deprecated">
  The standalone `@getoma/sdk` package is no longer maintained. Existing installs keep working, but new projects should use the official Anthropic SDKs as shown below.
</Aside>
...
The Stainless-generated SDKs in every language pass unknown body fields through unchanged — there is no separate `@getoma/sdk` to install. Use the SDK's standard escape hatch:
