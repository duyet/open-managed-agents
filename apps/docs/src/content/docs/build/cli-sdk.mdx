---
title: CLI & SDK
description: The open-managed-agents Node CLI and using @anthropic-ai/sdk against oma — your two ways to talk to the API without curl.
---

import { Tabs, TabItem, Aside, LinkCard } from '@astrojs/starlight/components';

oma is callable via two routes:

- **`@duyet/oma-cli`** — a Node CLI for scripting against the API.
- **Anthropic's official SDKs** — TypeScript ([`@anthropic-ai/sdk`](https://www.npmjs.com/package/@anthropic-ai/sdk)), Python ([`anthropic`](https://pypi.org/project/anthropic/)), and Go ([`anthropic-sdk-go`](https://pkg.go.dev/github.com/anthropics/anthropic-sdk-go)) — pointed at oma's `baseURL`. The oma API is wire-compatible with the Claude Managed Agents API, so the same SDKs work against both.
...
<Tabs syncKey="pkg-mgr">
  ...
  <TabItem label="pnpm">
    ```bash
    pnpm add -g @duyet/oma-cli
    ```
  </TabItem>
  <TabItem label="bun">
    ```bash
    bun add -g @duyet/oma-cli
    ```
  </TabItem>
</Tabs>
...
```bash
export OMA_BASE_URL="https://app.oma.duyet.net"
...
oma sessions chat sess_xyz "Summarize the oma docs."
...
The oma API is **wire-compatible with the [Claude Managed Agents API](https://docs.anthropic.com/)**, so the recommended client is one of Anthropic's official SDKs pointed at our `baseURL`. One SDK, two providers — moving between them is a config line.

<Aside type="caution" title="@duyet/oma-sdk is deprecated">
  The standalone `@duyet/oma-sdk` package is no longer maintained. Existing installs keep working, but new projects should use the official Anthropic SDKs as shown below.
</Aside>
...
<Tabs syncKey="lang">
  <TabItem label="TypeScript">
    ...
    const client = new Anthropic({
      baseURL: "https://app.oma.duyet.net",
  ...
  <TabItem label="Python">
    ...
    client = Anthropic(
        base_url="https://app.oma.duyet.net",
  ...
  <TabItem label="Go">
    ...
    client := anthropic.NewClient(
        option.WithBaseURL("https://app.oma.duyet.net"),
...
Every namespace in the official SDK works against oma:
...
The Stainless-generated SDKs in every language pass unknown body fields through unchanged — there is no separate `@duyet/oma-sdk` to install. Use the SDK's standard escape hatch:
