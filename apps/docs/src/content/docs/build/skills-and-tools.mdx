...
`web_fetch` and `web_search` route through oma's outbound proxy, so they work without the sandbox having direct internet access.
...
Connect any [Model Context Protocol](https://modelcontextprotocol.io) server and oma auto-generates an `mcp__<server>__<tool>` (double underscore) for every capability the server exposes.
