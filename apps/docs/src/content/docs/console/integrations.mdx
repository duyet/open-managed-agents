...
<Aside type="tip" title="One-click managed install">
  If the deployment has a shared managed Slack App configured (`SLACK_MANAGED_CLIENT_ID` / `SLACK_MANAGED_CLIENT_SECRET` / `SLACK_MANAGED_SIGNING_SECRET`), the publish wizard shows an **Add to Slack** button that skips the manifest / paste-credentials steps entirely — one click straight to the OAuth authorize screen (`POST /v1/integrations/slack/start-managed`). Without those secrets it falls back to the bring-your-own-App manifest wizard described below.
</Aside>
