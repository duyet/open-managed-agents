---
title: Contributing
description: How the oma repo is laid out, how to run it locally, where the design RFCs live, and how to ship a PR.
---

import { Steps, Aside, LinkCard } from '@astrojs/starlight/components';

Thanks for considering a contribution. oma is open source under the LICENSE in the repo root; PRs are welcome for features, bug fixes, docs, and integrations.
...
3. **Set up local secrets.**
   ...
   Copy `.dev.vars.example` to `.dev.vars` and fill in at least `PLATFORM_ROOT_SECRET` (`openssl rand -base64 32`), `BETTER_AUTH_SECRET` (`openssl rand -hex 32`), and `API_KEY`. `ANTHROPIC_API_KEY` is optional — set it to skip Model Card setup for local dev, or add a Model Card from the Console instead. The other secrets are optional locally.
