...
description: Run oma as a single Node process or docker compose stack — no Cloudflare account, no Workers, no Durable Objects, no Containers. SQLite or Postgres backend, local or remote sandbox.
---

import { Aside, Card, CardGrid, LinkCard, Tabs, TabItem } from '@astrojs/starlight/components';

If you want oma without a Cloudflare account — on a VPS, your laptop,
...
<Aside type="note">
  This page covers the **Node-side build** (`apps/main-node` — the self-host Node.js server, the same control-plane API as the `apps/main` Cloudflare Worker, packaged for `docker compose`). For the
...
**Same `oma/main-node:dev` image either way** — `DATABASE_URL` env at
...
oma now supports **multiple sandbox providers simultaneously** — not one
global `SANDBOX_PROVIDER`. Providers can be seeded from env vars at startup
(system providers) or added via the API at runtime (BYOK — bring your own key).
...
| `litebox` | Local hardware isolation, no daemon | Host needs `/dev/kvm` (Linux) or Apple Silicon (macOS). Per-VM Firecracker-equivalent via libkrun. |
| `boxrun` | Multi-host with KVM, oma pods on managed k8s | `BOXRUN_URL=http://host:8100/v1/default` (+ optional `BOXRUN_TOKEN`). Run `boxlite serve` on a separate KVM host; oma pods reach it over HTTP. |
| `daytona` | Production / untrusted code | `DAYTONA_API_KEY=...` (+ optional `DAYTONA_API_URL`). Daytona-managed Linux VMs, one per session. |
| `e2b` | E2B SaaS / E2B Infra self-host / Aliyun Agent Sandbox (e2b-compatible) | `E2B_API_KEY=...` (+ optional `E2B_API_URL` for non-default endpoint). Firecracker microVMs. |
| `browser-vm` (listed, not usable here) | N/A — Cloudflare only | Seeded from the same shared provider list, but `GET /v1/hosting_types` reports it `not_configured` on main-node — the RuntimeRoom WebSocket relay it needs is Cloudflare-only. |
...
Register additional sandbox providers at runtime via the API — useful for
multi-tenant scenarios where each tenant brings their own sandbox credentials:
...
Environments — the reusable packages/networking/image config sessions attach
to — now carry their own sandbox provider reference. Set
`config.sandbox_provider` to any registered provider ID (system or BYOK):
...
The Console's environment-create picker queries `GET /v1/hosting_types` to
show what's available — including any user-registered BYOK providers.
Fallback chain: `config.sandbox_provider` → `config.type` (legacy) → `SANDBOX_PROVIDER` env → `subprocess`.
