...
Activation is config-only — the wrangler files ship with `env.production` overlays that bind 4 sharded auth DBs (used by oma.duyet.net's hosted prod). To enable for your own deployment:

1. Provision the additional D1s: `wrangler d1 create openma-router`, `wrangler d1 create openma-auth-shard-01..03`
2. Patch the IDs into `env.production` blocks of `apps/{main,agent,integrations}/wrangler.jsonc` (replacing oma.duyet.net's IDs)
