...
3. Apply migrations to each shard:
   ...
   # openma-router needs the router tables, which live only in migrations-router/
   # (the auth consolidated migration does NOT carry them) — pass --migrations-dir:
   wrangler d1 migrations apply openma-router --remote --config apps/main/wrangler.jsonc --migrations-dir apps/main/migrations-router
   ```
