...
By default, oma is **multi-tenant on a single D1**. Every user gets a `tenant_id` row; every query is scoped by it. Adequate for most teams.
...
Activation is config-only, but **not pre-wired**: `env.production`'s `d1_databases` today bind only `MAIN_DB` + `INTEGRATIONS_DB` (single-D1 — oma.duyet.net launched this way since sharding is premature with zero users). To enable sharding for your own deployment, you re-add the `ROUTER_DB`/`AUTH_DB_00..03` bindings yourself:

1. Provision the additional D1s: `wrangler d1 create oma-router`, `wrangler d1 create oma-auth-shard-01..03`
2. Add `ROUTER_DB`/`AUTH_DB_00..03` entries (with the new ids) to the `env.production.d1_databases` array in `apps/{main,agent,integrations}/wrangler.jsonc`. Give the `ROUTER_DB` entry a `"migrations_dir": "migrations-router"` field — `wrangler d1 migrations apply` has no `--migrations-dir` CLI flag; the directory comes only from the matching `d1_databases` entry's own `migrations_dir` field (defaults to `./migrations`, relative to the config file, when unset).
3. Apply migrations to each shard (`--env production` selects the overlay you just edited):
   ```bash
   wrangler d1 migrations apply oma-auth-shard-01 --remote --config apps/main/wrangler.jsonc --env production
   wrangler d1 migrations apply oma-auth-shard-02 --remote --config apps/main/wrangler.jsonc --env production
   wrangler d1 migrations apply oma-auth-shard-03 --remote --config apps/main/wrangler.jsonc --env production
   # oma-router needs the router tables (tenant_shard/shard_pool/memory_store_tenant),
   # which live only in migrations-router/ — resolved via that entry's migrations_dir above.
   wrangler d1 migrations apply oma-router --remote --config apps/main/wrangler.jsonc --env production
   ```
4. Seed the shard pool:
   ```bash
   wrangler d1 execute oma-router --remote --command \
...
git pull
pnpm install
npx wrangler d1 migrations apply oma-auth         --remote --config apps/main/wrangler.jsonc
npx wrangler d1 migrations apply oma-integrations --remote --config apps/main/wrangler.jsonc
...
Every session gets its own Cloudflare Container instance via the `SANDBOX` Durable Object class. Sessions never share container state. The container's outbound network goes through oma's egress proxy; secrets (Vault entries) are injected per-host so the model never sees raw credentials.
...
This bites self-hosters using `curl` from automation against `*.oma.duyet.net` defaults; the User-Agent path is recorded in the [project memory](/contribute/) for hosted use.

### "Migration failed" on first deploy

Make sure you ran `wrangler d1 migrations apply oma-auth --remote` before `pnpm deploy`. The deploy script doesn't re-apply migrations.
