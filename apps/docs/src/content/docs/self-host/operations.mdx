...
3. Apply migrations to each shard (`--env production` selects the overlay you just edited):
   ```bash
   wrangler d1 migrations apply openma-auth-shard-01 --remote --config apps/main/wrangler.jsonc --env production
   wrangler d1 migrations apply openma-auth-shard-02 --remote --config apps/main/wrangler.jsonc --env production
   wrangler d1 migrations apply openma-auth-shard-03 --remote --config apps/main/wrangler.jsonc --env production
   # openma-router needs the router tables (tenant_shard/shard_pool/memory_store_tenant),
   # which live only in migrations-router/ — resolved via that entry's migrations_dir above.
   wrangler d1 migrations apply openma-router --remote --config apps/main/wrangler.jsonc --env production
   ```
