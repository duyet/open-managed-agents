...
description: Two ways to self-host oma — on Cloudflare Workers, or as a single Node process. Same code, same API, different I/O.
---

import { Card, CardGrid, Aside, LinkCard } from '@astrojs/starlight/components';

You can run all of oma — the API, the Console, the integrations gateway, and the agent runtime — yourself. The same code that powers [oma.duyet.net](https://app.oma.duyet.net) is the code in the public repo; nothing is held back.
...
<CardGrid>
  ...
  <Card title="A domain" icon="seti:html">
    You'll point routes at it. `oma.duyet.net` in our deployment; whatever you own in yours.
  </Card>
...
There are no oma platform fees because there is no oma platform — it's all your Cloudflare account.
