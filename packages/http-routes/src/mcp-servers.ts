 ...
 *
 * Storage: `services.kv` under `mcp_registry:<tenant>:<id>`. A registry is a
 ...
 * there is no index to keep consistent. `KvStore` is runtime-agnostic
 * (CfKvStore on Cloudflare, SqlKvStore on self-host Node), so this route
 * mounts identically on both runtimes — see apps/main/src/index.ts and
 ...
 * the proxy calls `resolveRegisteredMcpServer` to expand it into a URL
 * (+ optional credential_id) before resolving the vault token.
 */
 ...
 * proxy (CF's routes/mcp-proxy.ts and Node's mcp-proxy.ts) can expand
 * `agent.mcp_servers[].registry_id` → URL + credential_id at request time.
 * Returns null on miss / malformed row.
 */
...
export function buildMcpServerRoutes(deps: McpServerRoutesDeps) {
  ...
  app.post("/", async (c) => {
    ...
    await kv.put(kvKey(tenantId, row.id), JSON.stringify(row));
  ...
  app.get("/:id", async (c) => {
    const tenantId = c.get("tenant_id");
    const kv = resolveServices(deps.services, c).kv;
    const raw = await kv.get(kvKey(tenantId, c.req.param("id")));
  ...
  app.patch("/:id", async (c) => {
    ...
    await kv.put(kvKey(tenantId, id), JSON.stringify(row));
  ...
  app.delete("/:id", async (c) => {
    ...
    const raw = await kv.get(kvKey(tenantId, id));
    if (!raw) return c.json({ error: "not found" }, 404);
    await kv.delete(kvKey(tenantId, id));
