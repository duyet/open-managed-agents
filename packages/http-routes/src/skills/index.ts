// /v1/skills — custom + built-in Anthropic skill catalog.
//
// Ported from apps/main/src/routes/skills.ts (CF-local). Storage is
// runtime-agnostic: metadata lives in `services.kv` (same KvStore CF and
// Node both wire), file bytes live in `services.filesBlob` (R2 on CF,
// local-FS/S3 on self-host Node). Upload rate/size gates are an OPTIONAL
// `deps.quota` (QuotaService) — when absent, uploads soft-pass (no reject),
// matching the "all three soft-pass when their underlying primitive is
// absent" contract in @duyet/oma-quotas.

import { Hono } from "hono";
import { generateId, skillFileR2Key, logWarn } from "@duyet/oma-shared";
import { unzipSync } from "fflate";
import { listAll as kvListAll } from "@duyet/oma-kv-store";
import type { BlobStore } from "@duyet/oma-blob-store";
import type { QuotaService } from "@duyet/oma-quotas";
import type { RouteServicesArg, RouteServices } from "../types";
...
export interface SkillRoutesDeps {
  ...
  /** Optional upload-abuse gates (size + per-tenant frequency). Absent =
   *  soft-pass — every check becomes a no-op. */
  quota?: QuotaService;
}
...
interface SkillVersion {
  version: string;
  /** Manifest of files. Bytes live in the blob store at
   *  skillFileR2Key(t, id, ver, filename). */
...
const kvKey = (tenantId: string, ...parts: string[]) => `t:${tenantId}:${parts.join(":")}`;
const kvPrefix = (tenantId: string, ...parts: string[]) => `t:${tenantId}:${parts.join(":")}:`;

/**
 * Persist all files for a skill version to the blob store.
 */
async function writeFilesToBlob(
  bucket: BlobStore,
  tenantId: string,
  skillId: string,
  version: string,
...
async function readFilesFromBlob(
  bucket: BlobStore,
  tenantId: string,
  skillId: string,
  version: string,
...
async function deleteFilesFromBlob(
  bucket: BlobStore,
  tenantId: string,
  skillId: string,
  version: string,
...
function ensureBucket(services: RouteServices): BlobStore | null {
  return services.filesBlob ?? null;
}
...
> {
  ...
  const manifest = await writeFilesToBlob(bucket, tenantId, id, versionId, args.files);
  ...
  const filesOut = await readFilesFromBlob(bucket, tenantId, id, versionId, manifest);
...
> {
  ...
  const manifest = await writeFilesToBlob(bucket, tenantId, skillId, versionId, args.files);
  const version: SkillVersion = { version: versionId, files: manifest, created_at: now };
...
export function buildSkillRoutes(deps: SkillRoutesDeps) {
  ...
  app.post("/", async (c) => {
    ...
    const sizeCheck = deps.quota?.checkUploadSize(cl);
    if (sizeCheck?.reject) return c.json({ error: sizeCheck.message }, sizeCheck.status as 413);
    const freqCheck = await deps.quota?.checkUploadFreq(t);
    if (freqCheck?.reject) return c.json({ error: freqCheck.message }, freqCheck.status as 429);

    const bucket = ensureBucket(services);
    if (!bucket) return c.json({ error: "FILES_BUCKET binding not configured" }, 500);

    const body = await c.req.json<PersistArgs>();
    const result = await persistNewSkill(services, bucket, t, body);
  ...
  app.post("/upload", async (c) => {
    ...
    const result = await persistNewSkill(services, bucket, t, {
      files: parsed.files,
      name: parsed.name,
  ...
  // ---------------------------------------------------------------------
  // DELETE /v1/skills/:id — delete skill, all versions, and all blob objects
  // ---------------------------------------------------------------------
    ...
    if (bucket) {
      for (const k of versionKeys) {
        ...
        try {
          ...
          await deleteFilesFromBlob(bucket, t, id, v.version, v.files);
        } catch (err) {
          logWarn(
            { op: "skills.delete.r2_cleanup", tenant_id: t, skill_id: id, kv_key: k.name, err },
            "skill version blob cleanup failed; KV row will still be deleted",
          );
        }
      }
    }
  ...
  app.post("/:id/versions", async (c) => {
    ...
    const id = c.req.param("id");
    const body = await c.req.json<PersistArgs>();
    const result = await persistNewVersion(services, bucket, t, id, body);
  ...
  app.post("/:id/versions/upload", async (c) => {
    ...
    const id = c.req.param("id");
    const result = await persistNewVersion(services, bucket, t, id, {
      files: parsed.files,
      display_title: displayTitle,
      description: parsed.description,
    });
  ...
  app.get("/:id/versions/:version", async (c) => {
    ...
    const filesOut = await readFilesFromBlob(bucket, t, id, version, v.files);
  ...
  app.delete("/:id/versions/:version", async (c) => {
    ...
    const bucket = ensureBucket(services);
    if (bucket) await deleteFilesFromBlob(bucket, t, id, version, v.files);
