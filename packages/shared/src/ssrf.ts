// Shared SSRF guard for harness/worker-process fetches of externally-
// influenced URLs — model-supplied (web_fetch, browser_navigate) or
// tenant-configured (notify webhook.url, MCP server upstreams). Validates a
// URL BEFORE the harness/worker process fetches it directly — not
// sandbox-side execution (a sandboxed curl fallback is a separate,
// sandbox-network-policy concern and is left alone here).
//
// Lives in @duyet/oma-shared (rather than apps/agent) so packages that
// don't depend on apps/agent — e.g. @duyet/oma-browser-harness — can reuse
// it too. apps/agent/src/harness/ssrf.ts re-exports this module unchanged
...
export interface AssertPublicUrlOptions {
   ...
   *  scheme check is still always enforced). Callers wire this to their own
   *  operator escape hatch — e.g. WEB_FETCH_ALLOW_PRIVATE for web_fetch,
   *  MCP_ALLOW_PRIVATE_UPSTREAMS for the MCP proxy — for deployments that
   *  intentionally want internal reachability. */
...
/**
 * Validate that `url` is safe for the HARNESS/WORKER PROCESS (not the
 * sandbox) to fetch directly. Throws with a human-readable message if the
 * URL should be blocked; returns the parsed URL otherwise.
 *
